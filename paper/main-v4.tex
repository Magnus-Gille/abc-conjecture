...
iteration is new.  For odd \(c\), the exact one-step formula
\[
 \rad\bigl((b-a)^2(4ab)c^2\bigr)
 =\rad(|b-a|)\rad(abc)
\]
follows immediately from the disjoint prime supports proved in
Lemma~\ref{lem:support} (take \(n=1\)); no external result is needed
...
bound for Lucas and Lehmer sequences.  In Stewart's notation
\(Q(N)=\rad(|N|)\), \(q(m)\) is the number of square-free divisors of
\(m\), \(d(m)\) is the number of positive divisors, and \(|2|_2=1/2\).
For the companion sequence his displayed inequality, after taking
logarithms, is
\[
 \log Q(v_m)>
 \frac{c_{\!S}\,d(m|m|_2)(\log m)^2}
      {q(m)\log\log m}
\]
for an effective constant \(c_{\!S}>0\) and all sufficiently large
\(m\).  At \(m=2^k\), one has \(q(m)=2\), \(m|m|_2=1\), and
\(d(m|m|_2)=1\).  Since \(\log m=k\log2\) and
\(\log\log m\asymp\log k\), this gives
\[
...
Anthropic Claude Fable~5 and Opus~5, together with OpenAI Codex
(GPT-5-based service model), using the service versions available on
July 26, 2026, were used for idea exploration, candidate proofs,
literature classification and retrieval, coding assistance, drafting,
and independent adversarial review.  They were used to accelerate
exploration and to expose errors through independent cross-checking.
Mathematical claims were
